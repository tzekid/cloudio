...
- DB-backed actual route-capture planning with pre-limit `review_summary` totals plus candidate-level `actual_state`, `review_status`, and `next_action` fields for family triage.
