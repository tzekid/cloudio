...
Cloudflare control-plane mutation routes can also contribute `generated_dry_run_policy_evidence` when the checked manifest route is non-deprecated, routable, belongs to a Cloudio control-plane family, and remains `unsafe_mutation` / `dry_run`. This evidence is still no-execute L1 review evidence only: generic provider dispatch may render `will_execute:false` mutation plans for those routes, but live write support remains absent until a separate write-mode policy exists. Hostinger mutations and Cloudflare mutations outside the control-plane classifier stay visible as dry-run candidates unless explicit fixture or typed dry-run evidence closes them.
