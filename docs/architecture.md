...
- `providers/routes.zig` owns generated-manifest-backed route metadata for Cloudflare and Hostinger: provider, tag, method, path template, operation ID, support/mode, deprecation state, official path/query parameter requirements, request-body requirements, response status/content/schema metadata, path-placeholder extraction, required path/query validation, path-template metadata invariants, one reusable path/query request object, and escaped path/query/URL rendering. This is the L1 bridge for broad provider coverage and future generic dispatch.
