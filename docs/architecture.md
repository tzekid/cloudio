...
- `providers/routes.zig` owns generated-manifest-backed route metadata for Cloudflare and Hostinger: provider, tag, method, path template, operation ID, support/mode, deprecation state, official path/query parameter requirements, path-placeholder extraction, and escaped path/URL rendering. This is the L1 bridge for broad provider coverage and future generic dispatch.
- `providers/dispatch.zig` owns provider-neutral L1 dispatch. It renders routes through `providers/routes.zig`, applies Cloudflare or Hostinger auth through the existing provider clients, executes only `GET`/`read` routes, and renders mutation routes as `will_execute:false` dry-run plans without issuing HTTP.
