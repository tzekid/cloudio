...
- `app/coverage.zig` owns human-facing inspection of generated route contracts, including tag/status/mode filters, exact route selection by operation ID or method/path, no-execute route planning, metadata-only route reads, and generic route capture into `snapshots`, `provider_raw`, `audit_events`, and broad provider inventory tables through the shared collector capture helper plus provider resource parsers. Paginated capture is generic but conservative: it requires generated `page` or `cursor` query metadata, stores each page as its own redacted snapshot/raw row, recognizes Cloudflare offset `result_info`, Cloudflare cursor `result_info`, Cloudflare root cursor, and Hostinger `data/meta` page envelopes through `net/pagination.zig`, reports the recognized envelope and normalized resource count in metadata, redacts cursor tokens from emitted endpoints and audit labels, and stops on non-2xx, missing page-envelope metadata, no next page, or the max-page cap. CLI coverage and route commands should stay thin over this app surface so future UI/API views can reuse the same route addressing, request planning, redacted capture, and inventory behavior.
