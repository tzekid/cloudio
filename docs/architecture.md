...
- CLI commands are thin adapters. `cli/root.zig` should stay limited to top-level dispatch; command-group modules parse their own subcommands, call collectors or query services, and render output. Data output goes through `cli/render.zig` to stdout so text and JSON surfaces can be piped predictably; shared CLI output-format parsing uses `cli_render.RenderFormat` so `--json` and `--format` semantics do not drift between command groups. Diagnostics and usage errors may continue to use stderr.
