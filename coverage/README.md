...
zig build run -- coverage capture-candidates cloudflare --family security --limit 10 --plans --json
...
zig build run -- coverage dry-run-candidates cloudflare --family security --limit 10 --plans --json
...
`cloudio coverage typed-models [all|cloudflare|hostinger] [--family <family>] [--limit <n>]` ranks tag groups where generic L3 inventory rows exist but typed SQLite projections are missing or thin, and emits route/workplan commands for the next broad modeling slice. Add `--include-complete` to include groups that already have typed-table evidence.
