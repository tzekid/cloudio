...
- `security` is generated from operation-level OpenAPI security with root security fallback and records whether auth is required plus acceptable scheme alternatives. Generated rows keep Cloudflare combined auth scheme bundles intact; provider dispatch owns the explicit token-or-legacy compatibility mapping.
