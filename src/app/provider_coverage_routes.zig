...
pub const WorkplanFamily = app_provider_family.WorkplanFamily;
...
pub fn tagIsControlPlane(provider: []const u8, tag: []const u8) bool {
    return app_provider_family.tagIsControlPlane(provider, tag);
}

pub fn tagFamily(provider: []const u8, tag: []const u8) ?WorkplanFamily {
    return app_provider_family.tagFamily(provider, tag);
}
