...
test "hostinger app renders account overview across provider families" {
    ...
    try std.testing.expectEqual(@as(usize, 132), summary.api_routes);
    try std.testing.expectEqual(@as(usize, 53), summary.api_read_routes);
    try std.testing.expectEqual(@as(usize, 76), summary.api_dry_run_routes);
    ...
    try std.testing.expect(std.mem.indexOf(u8, text, "api_routes total=132 read=53 dry_run=76 write=0 not_applicable=1 deprecated=2 blocked_permission=6 families=8 observed_read_families=7 missing_read_families=0 not_applicable_families=1 blocked_or_missing_families=0") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, text, "billing\tofficial_routes=9\tread_routes=3\tdry_run_routes=6\twrite_routes=0\tnot_applicable_routes=0\tdeprecated_routes=0\tblocked_permission_routes=0") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"label\":\"billing\",\"official_routes\":9,\"read_routes\":3,\"dry_run_routes\":6,\"write_routes\":0,\"not_applicable_routes\":0,\"deprecated_routes\":0,\"blocked_permission_routes\":0") != null);
