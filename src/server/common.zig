...
pub fn mapApiError(err: anyerror) ApiErrorResponse {
    return switch (err) {
        ...
        error.InvalidSecretSourceKind, error.InvalidSecretSourceRef, error.SecretSourcePathNotAbsolute, error.InvalidEnvironmentName, error.ReservedEnvironmentName, error.InvalidSecretFile, error.InvalidSecretValue => .{ .status = 400, .body = "{\"error\":\"invalid_secret_binding\"}\n" },
