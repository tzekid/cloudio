...
pub const SummaryCommand = struct {
    format: RenderFormat = .text,
};

pub const SourceCommand = struct {
...
pub const GapCommand = struct {
    options: app_coverage.GapOptions = .{},
    format: RenderFormat = .text,
};

pub const LevelCommand = struct {
    provider: app_coverage.ProviderFilter = .all,
    format: RenderFormat = .text,
};

pub const L1Command = struct {
    provider: app_coverage.ProviderFilter = .all,
    format: RenderFormat = .text,
};

pub const LevelTagCommand = struct {
    options: app_coverage.LevelTagOptions = .{},
    format: RenderFormat = .text,
};

pub const FamilyCommand = struct {
    options: app_coverage.FamilyOptions = .{},
    format: RenderFormat = .text,
};

pub const TypedModelsCommand = struct {
    options: app_coverage.TypedModelOptions = .{},
    format: RenderFormat = .text,
};

pub const WorkplanCommand = struct {
    options: app_coverage.WorkplanOptions = .{},
    format: RenderFormat = .text,
};

pub const RouteCommand = struct {
    filter: app_coverage.RouteFilter = .{},
    format: RenderFormat = .text,
};

pub const CaptureCandidateCommand = struct {
    options: app_coverage.CaptureCandidateOptions = .{},
    format: RenderFormat = .text,
};

pub const ActualCaptureCommand = struct {
    options: app_coverage.ActualCaptureOptions = .{},
    format: RenderFormat = .text,
};

pub const DryRunCandidateCommand = struct {
...
pub const usage_text =
    ...
    \\  cloudio coverage typed-models [all|cloudflare|hostinger] [all|control-plane|<family>] [--focus all|control-plane] [--family <family>] [--limit <n>] [--include-complete] [--json|--format json]
...
test "coverage command parser defaults to summary" {
    ...
    const typed_models_focus_args = [_][]const u8{ "typed-models", "cloudflare", "control-plane", "--limit=0", "--json" };
    switch (parseCommand(typed_models_focus_args[0..])) {
        .typed_models => |command| {
            try std.testing.expectEqual(app_coverage.ProviderFilter.cloudflare, command.options.provider);
            try std.testing.expectEqual(app_coverage.WorkplanFocus.control_plane, command.options.focus);
