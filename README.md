...
`route plan` never sends HTTP. `route read` executes only generated bodyless `GET`/read routes with the configured provider credentials and prints response metadata without response bodies. `route capture` executes the same safe read path, redacts the response once, stores it in `snapshots` and `provider_raw`, writes a `route.capture` audit event, and still prints metadata only. `route dry-run` renders mutation plans with `will_execute:false`; it validates path/query/header/body metadata but never sends a provider write.
