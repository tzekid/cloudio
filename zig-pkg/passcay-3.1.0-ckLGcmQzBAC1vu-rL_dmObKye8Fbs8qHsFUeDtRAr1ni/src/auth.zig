...
/// for replay detection. The signature algorithm is taken from the credential
/// public key's COSE `alg` (the full registry in `alg.zig`).
///
...
pub fn verify(allocator: mem.Allocator, input: AuthVerifyInput, expectations: AuthVerifyExpectations) !AuthVerifyResult {
    ...
    const cd_bytes = try util.decodeClientDataJson(allocator, input.client_data_json);
    defer cd_bytes.deinit(allocator);

    const client_data = try util.parseClientDataJsonBytes(allocator, cd_bytes.bytes);
    ...
    crypto.sha256(cd_bytes.bytes, &client_data_hash, .{});
...
test "verify with explicit challenge" {
    ...
    var auth_data_bytes = std.mem.zeroes([37]u8);
    var rp_id_hash: [32]u8 = undefined;
...
test "verify with memory leak detection" {
    ...
    var auth_data_bytes = std.mem.zeroes([50]u8);
    var rp_id_hash: [32]u8 = undefined;
    crypto.sha256("localhost", &rp_id_hash, .{});
    @memcpy(auth_data_bytes[0..32], &rp_id_hash);
...
test "parseAuthenticatorData with credential data flag" {
    ...
    var auth_data = std.mem.zeroes([50]u8); // Extended size
    var rp_id_hash: [32]u8 = undefined;
...
test "extension data flag detection" {
    ...
    var auth_data = std.mem.zeroes([50]u8);
    var rp_id_hash: [32]u8 = undefined;
    crypto.sha256("localhost", &rp_id_hash, .{});
    @memcpy(auth_data[0..32], &rp_id_hash);
...
test "verify skipping user presence verification" {
    ...
    var auth_data_bytes = std.mem.zeroes([37]u8); // Standard size
    var rp_id_hash: [32]u8 = undefined;
    crypto.sha256("localhost", &rp_id_hash, .{});
    @memcpy(auth_data_bytes[0..32], &rp_id_hash);
